...
CARDS = {
    ...
    # بطاقات ب٢ — الدفعة الثانية (2026-07-19)
    "bukhl-shuhh": {
        "title": "بخل / شح",
        "sides": OrderedDict([
            ("بخل", ["بَخِلَ", "بُخْل"]),
    ...
    "nima-naim": {
        ...
        "extra_sides": OrderedDict([("نَعمة (بالفتح)", ["نَعْمَة"])]),
    },
    "rafa-rahma": {
        "title": "رأفة / رحمة",
        "sides": OrderedDict([
            ("رأفة/رءوف", ["رَأْفَة", "رَءُوف"]),
            ("رحمة", ["رَحْمَة"]),
        ]),
    },
    "ahd-mithaq": {
        "title": "عهد / ميثاق",
        "sides": OrderedDict([
            ("عهد (الأسرة)", ["عَهْد", "عَهِدَ", "عاهَدَ"]),
            ("ميثاق (الأسرة)", ["مِيثاق", "مَوْثِق", "واثَقَ"]),
        ]),
    },
    "istafa-ijtaba": {
        "title": "اصطفى / اجتبى / اختار",
        "sides": OrderedDict([
            ("اصطفى", ["اصْطَفَى", "مُصْطَفَيْن"]),
            ("اجتبى", ["اجْتَبَى"]),
            ("اختار", ["اخْتارَ"]),
        ]),
    },
    ...
    "halafa-aqsama": {
        ...
        "sides": OrderedDict([
            ("حلف", ["حَلَفْ", "حَلّاف"]),
            ("أقسم", ["أَقْسَمُ", "قاسَمَ", "تَقاسَمُ"]),
        ]),
    },
    "khushu-khudu": {
        "title": "خشوع / خضوع",
        "sides": OrderedDict([
            ("خشوع", ["خاشِع", "خَشَعَتِ", "خُشُوع"]),
            ("خضوع", ["خاضِع", "تَخْضَعْ"]),
        ]),
    },
    "fitna": {
        ...
        "sides": OrderedDict([
            ("فتنة (الاسم)", ["فِتْنَة"]),
            ("فتن (الفعل وأسرته)", ["فَتَنُ", "فُتُون", "فاتِن", "مَفْتُون"]),
        ]),
    },
    "sakina": {
        "title": "السكينة",
...
def resolve_lemma_ids(cx, names):
    ...
    ids = []
    for name in names:
        # صيغة «الاسم#المعرف» تحسم الاشتباه الهيكلي التام (كظَنَّ الفعل/ظَنّ الاسم)
        if "#" in name:
