...
const squash = (s) => String(s).replace(STRIP, "").replace(/\s+/g, " ");
...
function unbackedQuote(text, hay) {
  ...
  for (const m of text.matchAll(/﴿([^﴾]*)﴾/g)) {
    const frags = m[1].split(/…|\.\.\./).map((f) => squash(f).trim()).filter((f) => f.length >= 10);
    if (frags.length && !frags.every((f) => hayQ.includes(f))) return `اقتباس قرآني: ﴿${m[1].slice(0, 50)}…﴾`;
  }
...
export default async function handler(req) {
  ...
  if (body?.finalize) {
    ...
    for (let attempt = 0; attempt < 2; attempt++) {
      const fviol = ftext ? unbackedQuote(ftext, hay) : null;
      if (!fviol) break;
  ...
  for (let attempt = 0; attempt < 2; attempt++) {
    const viol = text ? unbackedQuote(text, hay) : null;
    if (!viol) break;
